...
class Hand:
    ...
    def find_type(self) -> str:
        ...
        most_common_card, most_common_count = unique_cards.most_common(1)[0]

        if 'J' in self.hand:
            # JOKER!
            if most_common_card == 'J' and most_common_count != 5:
                mc = unique_cards.most_common()
                n = mc[1]
                most_common_card, most_common_count = n

            most_common_count += unique_cards['J']
            unique_cards[most_common_card] = most_common_count
        ...
        raise AssertionError(f'Could not find type for {self.hand}')
